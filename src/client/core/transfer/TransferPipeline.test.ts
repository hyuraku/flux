...
describe('Transfer Pipeline 統合テスト', () => {
  ...
  describe('暗号化あり転送', () => {
    /** 暗号化付き送信パイプライン: File -> split -> (compress) -> encrypt -> serialize */
    async function senderPipelineEncrypted(
      file: File,
      cs: number,
      compress: boolean,
      encryption: EncryptionService,
    ): Promise<Uint8Array[]> {
      const cm = new ChunkManager(cs);
      const compression = new CompressionService();
      ...
      for await (const chunk of cm.split(file)) {
        ...
        const encrypted = await encryption.encrypt(data.buffer as ArrayBuffer);
        data = new Uint8Array(encryption.serializeEncryptedData(encrypted));
        chunks.push(ChunkManager.serializeChunk({ ...chunk, data }));
      }
      return chunks;
    }

    /** 暗号化付き受信パイプライン: deserialize -> decrypt -> (decompress) -> addChunk -> toFile */
    async function receiverPipelineEncrypted(
      serializedChunks: Uint8Array[],
      metadata: ChunkMetadata,
      isCompressed: boolean,
      encryption: EncryptionService,
      ...
      for (const serialized of serializedChunks) {
        const chunk = ChunkManager.deserializeChunk(serialized);
        const encData = encryption.deserializeEncryptedData(chunk.data.buffer as ArrayBuffer);
        const decrypted = await encryption.decrypt(encData);
    ...
    async function setupEncryptionPair(): Promise<[EncryptionService, EncryptionService]> {
      const sender = new EncryptionService();
      const receiver = new EncryptionService();
      const senderKey = await sender.generateKeyPair();
      const receiverKey = await receiver.generateKeyPair();
      await sender.deriveSharedKey(receiverKey);
      await receiver.deriveSharedKey(senderKey);
      return [sender, receiver];
    }

    it('暗号化のみ（圧縮なし）のラウンドトリップ', async () => {
      const [senderEnc, receiverEnc] = await setupEncryptionPair();
      const content = 'Hello, encrypted world!';
      const file = new File([content], 'secret.txt', { type: 'text/plain' });
      const cm = new ChunkManager(chunkSize);
      const metadata = cm.createMetadata(file);

      const serializedChunks = await senderPipelineEncrypted(file, chunkSize, false, senderEnc);
      const received = await receiverPipelineEncrypted(serializedChunks, metadata, false, receiverEnc);

      expect(received.name).toBe('secret.txt');
      const receivedBytes = await blobToArray(received);
      const originalBytes = toArray(new TextEncoder().encode(content));
      expect(receivedBytes).toEqual(originalBytes);
    });

    it('暗号化+圧縮のラウンドトリップ', async () => {
      const [senderEnc, receiverEnc] = await setupEncryptionPair();
      const content = 'CompressAndEncrypt! '.repeat(1000);
      const file = new File([content], 'both.txt', { type: 'text/plain' });
      const cm = new ChunkManager(chunkSize);
      const metadata = cm.createMetadata(file);

      const serializedChunks = await senderPipelineEncrypted(file, chunkSize, true, senderEnc);
      const received = await receiverPipelineEncrypted(serializedChunks, metadata, true, receiverEnc);
    ...
    it('暗号化なしで暗号化データを受信→データ不一致', async () => {
      const [senderEnc] = await setupEncryptionPair();
      const content = 'ShouldFail';
      const file = new File([content], 'fail.txt', { type: 'text/plain' });
      const cm = new ChunkManager(chunkSize);
      const metadata = cm.createMetadata(file);

      const serializedChunks = await senderPipelineEncrypted(file, chunkSize, false, senderEnc);
      // 復号せずに受信（暗号化データをそのまま結合）
      const broken = await receiverPipeline(serializedChunks, metadata, false);
    ...
    it('1バイトファイルの暗号化転送', async () => {
      const [senderEnc, receiverEnc] = await setupEncryptionPair();
      const file = new File([new Uint8Array([42])], 'tiny.bin');
      const cm = new ChunkManager(chunkSize);
      const metadata = cm.createMetadata(file);
